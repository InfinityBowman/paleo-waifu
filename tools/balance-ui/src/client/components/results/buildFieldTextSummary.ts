...
export function buildFieldTextSummary(result: FieldResult): string {
  ...
  lines.push(
    `  Win Rate Spread: ${pct(scorecard.winRateSpread)} (${pct(scorecard.minWinRate)} - ${pct(scorecard.maxWinRate)})`,
  )
  ...
  lines.push(
    `  Strongest: ${pct(scorecard.maxWinRate)} | Weakest: ${pct(scorecard.minWinRate)}`,
  )

  // Creature Team Performance
  if (creatureTeamStats.length > 0) {
    const teamSorted = [...creatureTeamStats].sort(
      (a, b) => b.teamWinRate - a.teamWinRate,
    )
    lines.push('')
    ...
    for (const c of teamSorted.slice(0, 10)) {
      const delta =
        c.teamDelta > 0
          ? `+${(c.teamDelta * 100).toFixed(1)}`
          : (c.teamDelta * 100).toFixed(1)
      lines.push(
        `  ${c.name} (${c.role}) — Team: ${pct(c.teamWinRate)} | Solo: ${pct(c.soloWinRate)} | Δ: ${delta}pp`,
      )
    }
    ...
    const byDelta = [...creatureTeamStats].sort(
      (a, b) => b.teamDelta - a.teamDelta,
    )
    ...
    if (boosted.length > 0 || reliant.length > 0) {
      ...
      if (boosted.length > 0) {
        lines.push(
          `  Team-boosted (${boosted.length}): ${boosted
            .slice(0, 5)
            .map((c) => `${c.name} (+${(c.teamDelta * 100).toFixed(1)}pp)`)
            .join(', ')}`,
        )
      }
      if (reliant.length > 0) {
        lines.push(
          `  Solo-reliant (${reliant.length}): ${reliant
            .slice(0, 5)
            .map((c) => `${c.name} (${(c.teamDelta * 100).toFixed(1)}pp)`)
            .join(', ')}`,
        )
  ...
  if (teamAbilityImpact.length > 0) {
    const sortedAbilities = [...teamAbilityImpact].sort(
      (a, b) => b.teamWinRate - a.teamWinRate,
    )
    lines.push('')
    ...
    for (const a of sortedAbilities) {
      ...
      lines.push(
        `  ${a.name} (${a.abilityType}) — ${pct(a.teamWinRate)} team WR (${delta > 0 ? '+' : ''}${(delta * 100).toFixed(1)}pp) | ${a.creaturesWithAbility} creatures | ${a.sampleSize} games`,
      )
    }
  }
  ...
  if (teamRoleMatchup.length > 0) {
    const sortedRoles = [...teamRoleMatchup].sort(
    ...
    for (const r of sortedRoles) {
      ...
      lines.push(
        `  ${r.role}: ${pct(r.winRate)} team WR (${delta > 0 ? '+' : ''}${(delta * 100).toFixed(1)}pp) | ${r.sampleSize} games`,
      )
    }
  }
  ...
  if (synergyImpact.length > 0) {
    ...
    for (const s of sortedSynergies) {
      lines.push(
        `  ${s.synergy}: ${pct(s.avgWinRate)} with vs ${pct(s.avgWinRateWithout)} without (${s.delta > 0 ? '+' : ''}${(s.delta * 100).toFixed(1)}pp) | ${s.sampleSize} teams`,
      )
    }
  }

  // Composition Win Rates
  const compEntries = Object.entries(compWinRates).sort(
    ([, a], [, b]) => b.winRate - a.winRate,
  )
  ...
  // Formation Win Rates
  const formEntries = Object.entries(formationWinRates).sort(
    ([, a], [, b]) => b.winRate - a.winRate,
  )
