    ...
    <ResponsiveContainer width="100%" height={280}>
      <ScatterChart margin={{ top: 10, right: 20, bottom: 20, left: 10 }}>
            ...
            return (
              <div className="rounded-lg border border-border/50 bg-card px-3 py-2 text-xs shadow-md">
                ...
                <div className="text-muted-foreground">
                  {d.role} · Solo: {d.solo}% · Team: {d.team}% · Δ: {d.delta > 0 ? '+' : ''}{d.delta}pp
                </div>
              </div>
            )
          }}
        ...
        <ReferenceLine
          ...
          strokeDasharray="4 4"
        />
        <ReferenceLine x={50} stroke="oklch(1 0 0 / 8%)" strokeDasharray="2 4" />
        <ReferenceLine y={50} stroke="oklch(1 0 0 / 8%)" strokeDasharray="2 4" />
        {byRole.map((group) => (
          <Scatter
            key={group.role}
            name={group.role}
