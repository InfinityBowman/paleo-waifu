import { Link } from '@tanstack/react-router'
import { signIn, useSession } from '@/lib/auth-client'
import { Skull, Sparkles, BookOpen } from 'lucide-react'

export function Hero() {
  const { data: session } = useSession()

  return (
    <section className="relative overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-b from-primary/5 via-transparent to-transparent" />
      <div className="relative mx-auto max-w-4xl px-4 py-24 text-center sm:py-32">
        <div className="mb-6 inline-flex items-center gap-2 rounded-full border bg-card px-4 py-1.5 text-sm">
          <Sparkles className="h-4 w-4 text-primary" />
          Prehistoric creatures, reimagined
        </div>

        <h1 className="text-4xl font-bold tracking-tight sm:text-6xl">
          Collect{' '}
          <span className="bg-gradient-to-r from-amber-400 via-primary to-purple-400 bg-clip-text text-transparent">
            Prehistoric Waifus
          </span>
        </h1>

        <p className="mx-auto mt-6 max-w-xl text-lg text-muted-foreground">
          Dig up fossils, pull from gacha banners, and build your collection of
          waifu-fied dinosaurs, marine reptiles, and ancient mammals. Trade with
          other collectors to catch &apos;em all.
        </p>

        <div className="mt-10 flex items-center justify-center gap-4">
          {session ? (
            <Link
              to="/gacha"
              className="inline-flex h-11 items-center gap-2 rounded-lg bg-primary px-6 font-medium text-primary-foreground hover:bg-primary/90"
            >
              <Skull className="h-5 w-5" />
              Start Pulling
            </Link>
          ) : (
            <button
              onClick={() => signIn.social({ provider: 'discord' })}
              className="inline-flex h-11 items-center gap-2 rounded-lg bg-[#5865F2] px-6 font-medium text-white hover:bg-[#4752C4]"
            >
              <svg className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
                <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.947 2.418-2.157 2.418z" />
              </svg>
              Login with Discord
            </button>
          )}
          <Link
            to="/encyclopedia"
            className="inline-flex h-11 items-center gap-2 rounded-lg border px-6 font-medium hover:bg-accent"
          >
            <BookOpen className="h-5 w-5" />
            Encyclopedia
          </Link>
        </div>

        <div className="mt-16 grid grid-cols-3 gap-8 text-center">
          <div>
            <div className="text-3xl font-bold text-primary">100+</div>
            <div className="mt-1 text-sm text-muted-foreground">Creatures</div>
          </div>
          <div>
            <div className="text-3xl font-bold text-primary">5</div>
            <div className="mt-1 text-sm text-muted-foreground">Rarity Tiers</div>
          </div>
          <div>
            <div className="text-3xl font-bold text-primary">3</div>
            <div className="mt-1 text-sm text-muted-foreground">Geological Eras</div>
          </div>
        </div>
      </div>
    </section>
  )
}
