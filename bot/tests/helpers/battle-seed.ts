...
export async function seedBattleData() {
  ...
  await execute(
    `INSERT OR REPLACE INTO creature_battle_stats (creature_id, role, hp, atk, def, spd)
     VALUES (?, ?, ?, ?, ?, ?)`,
    TEST_CREATURE_ID_2, 'tank', 120, 30, 50, 20,
  )
  await execute(
    `INSERT OR REPLACE INTO creature_battle_stats (creature_id, role, hp, atk, def, spd)
     VALUES (?, ?, ?, ?, ?, ?)`,
    TEST_CREATURE_ID_3, 'striker', 80, 50, 20, 40,
  )
  await execute(
    `INSERT OR REPLACE INTO creature_battle_stats (creature_id, role, hp, atk, def, spd)
     VALUES (?, ?, ?, ?, ?, ?)`,
    TEST_CREATURE_ID_4, 'support', 90, 35, 35, 35,
  )
  ...
  await execute(
    `INSERT OR REPLACE INTO user_creature (id, user_id, creature_id, pulled_at)
     VALUES (?, ?, ?, ?)`,
    UC_USER1_CREATURE3, TEST_APP_USER_ID, TEST_CREATURE_ID_3, now,
  )
  await execute(
    `INSERT OR REPLACE INTO user_creature (id, user_id, creature_id, pulled_at)
     VALUES (?, ?, ?, ?)`,
    UC_USER1_CREATURE4, TEST_APP_USER_ID, TEST_CREATURE_ID_4, now,
  )
  ...
  await execute(
    `INSERT OR REPLACE INTO user_creature (id, user_id, creature_id, pulled_at)
     VALUES (?, ?, ?, ?)`,
    UC_USER2_CREATURE2, TEST_APP_USER_ID_2, TEST_CREATURE_ID_2, now,
  )
  await execute(
    `INSERT OR REPLACE INTO user_creature (id, user_id, creature_id, pulled_at)
     VALUES (?, ?, ?, ?)`,
    UC_USER2_CREATURE3, TEST_APP_USER_ID_2, TEST_CREATURE_ID_3, now,
  )
  await execute(
    `INSERT OR REPLACE INTO user_creature (id, user_id, creature_id, pulled_at)
     VALUES (?, ?, ?, ?)`,
    UC_USER2_CREATURE4, TEST_APP_USER_ID_2, TEST_CREATURE_ID_4, now,
  )

  // Team presets — members is JSON: [{userCreatureId, creatureId, row}]
  const user1Members = JSON.stringify([
    { userCreatureId: TEST_USER_CREATURE_ID, creatureId: TEST_CREATURE_ID_2, row: 'front' },
    { userCreatureId: UC_USER1_CREATURE3, creatureId: TEST_CREATURE_ID_3, row: 'front' },
    { userCreatureId: UC_USER1_CREATURE4, creatureId: TEST_CREATURE_ID_4, row: 'back' },
  ])
  await execute(
    `INSERT OR REPLACE INTO battle_team_preset (id, user_id, name, members, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?)`,
    PRESET_USER1_ID, TEST_APP_USER_ID, 'Team Alpha', user1Members, now, now,
  )

  const user2Members = JSON.stringify([
    { userCreatureId: UC_USER2_CREATURE2, creatureId: TEST_CREATURE_ID_2, row: 'front' },
    { userCreatureId: UC_USER2_CREATURE3, creatureId: TEST_CREATURE_ID_3, row: 'front' },
    { userCreatureId: UC_USER2_CREATURE4, creatureId: TEST_CREATURE_ID_4, row: 'back' },
  ])
  await execute(
    `INSERT OR REPLACE INTO battle_team_preset (id, user_id, name, members, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?)`,
    PRESET_USER2_ID, TEST_APP_USER_ID_2, 'Team Beta', user2Members, now, now,
  )
}
...
export async function seedPendingChallenge() {
  ...
  await execute(
    `INSERT OR REPLACE INTO battle_challenge (id, challenger_id, defender_id, status, challenger_team, created_at)
     VALUES (?, ?, ?, ?, ?, ?)`,
    TEST_CHALLENGE_ID, TEST_APP_USER_ID, TEST_APP_USER_ID_2, 'pending', team, now,
  )
}
